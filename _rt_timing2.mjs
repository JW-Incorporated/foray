...
const pool = fullPool();
// SIMULATE THE REAL APP: one persistent ctx object, like state._searchCtx
const ctx = { semantic, itemTags, discover };

const queries = ["nuclear fusion energy","startups and venture capital","true crime cold case","video games","politics","psychology","murder","climate change","self improvement","comedy","stock market","artificial intelligence"];

console.log("=== FIRST PASS (cold memo) ===");
...
for (const q of queries) {
  ...
  console.log(q, (Number(qt1-qt0)/1e6).toFixed(2), "ms");
}
let t1 = process.hrtime.bigint();
console.log("first pass total:", (Number(t1-t0)/1e6).toFixed(2), "ms");

console.log("=== SECOND PASS (warm memo, same ctx reused as app.js does) ===");
...
console.log("second pass total:", (Number(t1-t0)/1e6).toFixed(2), "ms");
