...
for (const q of ["stock market", "artificial intelligence", "board games"]) {
  const r = run(q);
  ...
  console.log(JSON.stringify({q, status:r.status, resultsN:r.results.length, strongN:strong.length,
    distinctStrongShows: distinctStrongShows.size, distinctResultShows: distinctResultShows.size, pickShowCounts}));
}

// Direct diversify() unit stress test: many distinct shows available but cap allegedly violated?
const cands = [];
for (let s = 0; s < 8; s++) {
  for (let e = 0; e < 5; e++) {
    cands.push({ i: { id: `show${s}-ep${e}`, show: `Show${s}` }, sum: 10 - s*0.1 - e*0.01, matched: 2 });
  }
}
const div = SE.diversify(cands, { cap: 10, perShowCap: 2 });
const counts = {};
div.forEach(c => counts[c.i.show] = (counts[c.i.show]||0)+1);
console.log("diversify() with 8 shows x5 eps each, cap10 perShowCap2:", JSON.stringify(counts));
