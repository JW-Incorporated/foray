...
Node.js + TypeScript backend core for 4a: feed ingest primitives,
...
| `npm run generate-foray` | **Generation pipeline §4.0-4.1** (`../docs/curation/generation-architecture.md`): captures a prompt + duration, runs it through the safety check, clarify step, and intent extraction, in that order. Founder-run CLI — see `docs/DECISIONS.md` 2026-09-01 for why. `npm run generate-foray -- --prompt "the history of grilling" --duration medium`. Zero API keys required — dry-run by default like everything else here. |
