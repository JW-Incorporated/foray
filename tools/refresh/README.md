...
`merge.mjs` no longer touches `deploy-manifest.json` or `sw.js` (issue #701).
The deploy stamp is a build output — `tools/web/prepare-dist.mjs` writes it into
`dist/` for Vercel and `.github/workflows/pages.yml` writes it for Pages — so the
nightly's commit is just the two data files, and it runs the same on a Windows
`core.autocrlf=true` checkout as on the nightly's Linux runner.
