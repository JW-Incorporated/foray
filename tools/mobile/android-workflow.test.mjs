...
test("ci.yml still declares exactly its four jobs, and #245 added none", () => {
  /* MUTATION: add a fifth job to ci.yml -> fails.
  ...
  const names = jobs
    ...
    .filter((l) => /^ {2}[a-z][\w-]*:/.test(l))
    .map((l) => l.trim().replace(":", ""));
  assert.deepEqual(names, ["backend", "ios-kit", "data-and-site", "playwright"]);
  assert.equal(
