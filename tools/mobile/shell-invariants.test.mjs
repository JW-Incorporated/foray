...
test("NE-19: one os.Logger (ai.jwlabs.foura / engine), whose only .public line is the gated DiagGate.loggerText, and nothing else in the engine logs", () => {
  ...
  for (const file of swiftFilesUnder(ENGINE_DIR)) {
    ...
    /* THE ONE EXCEPTION, NAMED: NE-15's AVDeck keeps its own primitive log
       (category ForayEngine.AVDeck), the deck-level trace the NE-25a/b spikes
       read off the Simulator. It predates the ring and writes nothing into it;
       routing it through DiagGate is a follow-up, not a licence for a third. */
    if (file === AVDECK_SWIFT) {
      assert.deepEqual([...code.matchAll(/Logger\(\s*subsystem:[^,]*,\s*category:\s*"([^"]*)"/g)].map((m) => m[1]), ["ForayEngine.AVDeck"]);
      continue;
    }
