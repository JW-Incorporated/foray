/* Applies data/genre-taxonomy-map.json to the breadth catalog(s), producing
   ...
   classification in the file — 1,851 of them when this was diagnosed,
   **19,278 plus 6,758 `superseded_topics` by the time the fix landed**, because
   PR #205 merged six stranded shard branches in between — and every entry for a
   show missing from the input catalog. That is why the file below is a merge,
   not a rebuild, and why the cost of leaving the unfixed version in `tools/`
   grew 10x in three weeks with nobody touching it.
