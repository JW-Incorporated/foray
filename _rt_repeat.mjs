...
// Simulate a user re-running buildPlaylist for the SAME query 5 times in a row
// (e.g. hitting "search" repeatedly, or app.js calling it from multiple UI spots)
const times = [];
for (let i=0;i<8;i++) {
  ...
  times.push(+(Number(t1-t0)/1e6).toFixed(1));
}
console.log("repeated identical-query buildPlaylist-equivalent timings (ms):", times);
