...
const FLOORS = {
  ...
  "player/foray-queue.test.js": 39, // NE-29j (OQ-6): a jingle counts on the Foray clock for its asset's measured 3.0 s, equal to the interlude's; 38 -> 39 // F-90 (2026-09-11): the jingle item asset is the interlude asset; 37 -> 38
  ...
  "player/parity/run.test.js": 40, // NE-31j: the narration overlay's driver (the bridge's opt-in shapes, the narration views, an awake clock, a silent synthesiser, dispose); 39 -> 40 // NE-30j: the engine target (T on every checkpoint, a refused command is a harness error), the manager's opt-in views and report projection, a real Foray build, and the deck's natural end; 38 -> 39 (NE-29s's build-table test had made it 38) // NE-28j: the deck target drives outPointStep over a driven clock (playhead = elapsed x rate, the watchdog delivered when due) and refuses what it cannot read; 36 -> 37 // NE-14j: the session and lifecycle drivers (interruption, route, cold launch, foreground), their refusals, held loads that land late, and positionEvents over the real PositionStore; 32 -> 36 // NE-12j: the media-actions adapter records real arity and refuses a press the OS could never deliver; 31 -> 32
  "player/parity/coverage.test.js": 33, // NE-31j: tts-bridge owes nothing, nothing is owed to NE-31j, the narration files are owed to NE-31s and speech-rate to NE-33, the card's spec cases are authored, and every spoken line is 1x; 32 -> 33 // NE-30j (31 -> 32 tests; the floor had one of slack): html-audio-backend and foray-playback owe nothing outside NE-31j/NE-39j, manager-foray / deck / prepare are owed to NE-30s (the pair's decisions to NE-32), prepare is authored against reference-engine, and foray-data is JS only; 30 -> 32 // NE-29j: foray-progress and media-session owe unported.json nothing, foray-clock / foray-progress / foray-structure / media are owed to NE-29s and default-voice to NE-33, and every never-4a case over a committed Foray is authored at zero; 29 -> 30 // NE-28j: seam-gap, interlude and seek-policy are wholly classified with zero unported entries, and every outpoint case (op-log scenarios included) is owed to NE-28s; 28 -> 29 // NE-21: transport-reconcile is wholly classified (a facade test, an exclusion, or owed to the engine's own cards), every facades.json mapping names a native-facades test that exists, and a mapping that could hide a Swift rule is refused; 25 -> 28 // NE-14j: queue-manager and html-audio-backend are wholly classified — manager-episode / deck-episode, an exclusion, an XCTest, or owed to NE-30j/NE-31j/NE-39j outside the episode capability; 24 -> 25 // NE-13: the continuation capability owes nothing, because its family is JS-only (plan C-2); 23 -> 24 // NE-12j: media-session is wholly classified — media-episode, an exclusion, or NE-29j's Foray half in the foray capability's family; 22 -> 23 // NE-07j: a suite whose recording card has landed (queue-state, playback-rate) owes nothing and is fixtured into its own family only; 21 -> 22
  ...
  /* NE-34's hash pin: the web jingle and the plugin's bundled copy hash to
     one SHA-256 that InterludePlayer.swift also carries, the plugin target
     ships that one resource, the audio guards' exemption for it is that
     narrow, and the App Review note says what the code does. Zero slack. */
  "tools/audio/interlude-asset.test.mjs": 4,
