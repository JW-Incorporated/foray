...
const FLOORS = {
  ...
  "player/media-session.test.js": 151, // founder 2026-09-23: the platform's seekOffset is ignored (15/30 are ours); the pause and the clear are reported writes; client.js records foray:remote; 147 -> 151 // 2026-09-22 audit (L2): a remote stop pauses and keeps the session; an episode's next/previous are the page's (the no-next pin was rewritten, R2); 146 -> 147 // 2026-09-22: client.js defers the shim-counter read past the shim's own flush (the founder's sent=0). The suite was already at 145 against a floor of 143 -- two cells had landed without a bump, so this closes that slack too; 143 -> 146 // 2026-09-21: the write and the report agree — a throwing metadata assignment is reported as failed and does not poison the dedupe; 140 -> 143 // client audit (2026-09-12): no raw NUL byte in player/*.js — a binary file is invisible to every search; 139 -> 140 // L-06 (2026-09-12): the Apple Podcasts parity rule + the onWrite hook; 132 -> 139 // F-89 (2026-09-11): a jingle item is credited to 4a; 131 -> 132
  ...
  "tools/mobile/foray-media-session.test.mjs": 84, // founder 2026-09-23: every native transport event is re-broadcast as foray:remote for the record (handled or not, named or not, and a hostile window cannot cost a press); the sends-in-turn probe sees the state write too; 80 -> 84 // 2026-09-22: `sends` cannot be read in the turn of the write, against the REAL default scheduler; 79 -> 80 // M-03 (2026-09-12): the session event reaches the page; 75 -> 79
