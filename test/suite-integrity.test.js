...
const FLOORS = {
  ...
  "player/parity/run.test.js": 38, // NE-30j: the engine target (T on every checkpoint, a refused command is a harness error), the manager's opt-in views and report projection, a real Foray build, and the deck's natural end; 37 -> 38 // NE-28j: the deck target drives outPointStep over a driven clock (playhead = elapsed x rate, the watchdog delivered when due) and refuses what it cannot read; 36 -> 37 // NE-14j: the session and lifecycle drivers (interruption, route, cold launch, foreground), their refusals, held loads that land late, and positionEvents over the real PositionStore; 32 -> 36 // NE-12j: the media-actions adapter records real arity and refuses a press the OS could never deliver; 31 -> 32
