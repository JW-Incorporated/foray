...
const FLOORS = {
  ...
  "test/playwright-fixture-server.test.js": 3, // round-3 audit, L4 (tests-1): new -- the Playwright fixture serves sw.js stamped with the served deploy id, so a 'new deploy' really re-installs
  ...
  "test/sw-generation.test.js": 58, // round-3 audit, L4: a late answer keeps a TRACKED file verified and writes an UNTRACKED one, awaited not ticked (app-3-14); 57 -> 58 // audit round 2 (L5): no self-rewrite of a tracked file, validators for untracked (perf-6 x2), unchanged files copied not fetched + no-cache, bad copy refused (perf-4 x2), fonts cache-first (perf-5); 52 -> 57 // S-03 (2026-09-12): +1 — cachePut's untracked-path branch is load-bearing now that data/show-index.tsv uses it; 51 -> 52
