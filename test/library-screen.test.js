...
/* Fable ruling (2026-09-06): the founder's later 2026 five-item drawer
   ...
   page carried its own `page-link-row` to #/library, shown only when the
   tab bar was not rendering (`!ui2On()`); U-11's cutover (founder
   override, 2026-09-06, kanban card t_a3f01c8a) retired cp_ui_v2 —
   ui2On() always returns true now, so that interim link never renders and
   the tab bar is the only entry point, for every listener. */
test("Library stays reachable via the tab bar (the interim Playlists link is retired dead code post-cutover)", () => {
  /* MUTATION: point the tab bar's Library entry at anything other than
     #/library, or make renderPlaylists() render the interim link again.
  ...
  assert.ok(
    !/<a class="page-link-row" href="#\/library">/.test(m.view()),
