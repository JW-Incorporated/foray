...
- **qa 152** was the last open row: `data/session.json`'s `fit_line` /
  ...
  2026-09-23 on `fix/qa152-session-copy`: the four fit-lines are plain duration
  statements (the shape `sessionBuilder.ts` already emits), the label is
  "Go deep", `COMMUTE_FRAMING` in `backend/src/copy/rules.js` names the shapes
  `BANNED` missed, and `copyRules.test.ts` reads every string on a card, with
