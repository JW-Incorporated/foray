...
| `mobile/ios/` | **Generated and BUILT** on a macOS runner — `BUILD SUCCEEDED` for simulator and arm64 device. #213, `docs/ios-ci.md` |
...
| **Either platform LAUNCHED on Android** | **No.** No emulator, no device. Every claim about a running Android app is still unverified — see §5 |
...
| iOS build | **MET.** `BUILD SUCCEEDED` for simulator and arm64 device on a macOS runner. #213, `docs/ios-ci.md` |
...
launched on Android, on any emulator or device. The iOS half of that gap is closed:
#213 measured `window.Capacitor` loading with **0 CSP violations** on both a
simulator and an arm64 device, so the `img-src 'self'` reasoning in §2.3 is now
observed rather than inferred *on iOS*. The Android device pass is
