...
| **`seam-gap.js`** | The seam decision table: which transitions get the authored beat (`SEAM_GAP_SEC`, 0.5 s since the founder's 2026-09-24 ruling; 2.0 s when this table was written) and which get none. | 128 | **17** / `seam-gap.test.js` | **Nothing at all.** | **Trivial. One screen of decision table, zero platform references, one import.** | **Full carry-over, 17 tests.** One caveat: `SEAM_GAP_SEC` is **authored** (0.5 s, the founder's 2026-09-24 ruling; it was 2.0 s from `docs/curation/segment-length-rules.md` §6b's audiobook section-break convention), not measured — so it is a constant that survives a platform change intact. Its *delivery* does not: the measured `observedGapMs: 9153` against `askedGapMs: 2000` is the seam defect, and it lives in the backend, not here. |
