...
| F-66 | `ttlA1Ms` measures the whole run — the partial candidate is written in stitch, after every act is narrated | Medium | card | WS-D follow-up (stitch act *i* when act *i* is narrated) | move the per-act partial write into the narration loop |
...
| F-68 | deepen paraphrases seeded claims; the overlap floor refuses the paraphrase — 5 tape of 16 seeded | Critical | fix in flight | `fix/f68-seeded-claims-frozen` (agent PR): claim frozen through deepen, restored mechanically, seed window first | attempt 5 measures the yield |
| F-69 | F-60's rephrased retry is an IDF word bag, not a query; 6 of 11 act-1 pages retried in attempt 4b | Medium | open | — | rephrase from the purpose sentence; skip the web for seeded beats |
