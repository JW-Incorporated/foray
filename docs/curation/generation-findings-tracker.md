...
| F-63 | the spine never reads the tape, so its beats claim what the archive does not say; matcher now refuses correctly | Critical | card | WS-L (tape-first spine) | build WS-L; re-run the run-2 prompt |
