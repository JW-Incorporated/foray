...
| F-73 | tape-first windows (60–120 s) fail the D-tier duration rules by construction (D3 mean ≥ 90 s, D5 IQR ≥ 45 s, D2 recovery) and M4's runtime clause; attempt 5's partial refused | Critical | fixed | #571 — research windows 90–180 s; tier-2 cut grows by claim overlap toward a length ladder (105/165/135/210 s, never across a > 5 s transcript gap); ledger gates `d3-mean`, `d2-short-run`, `m4-runtime`, and `d5-uniform` as a preference; constants mirrored from check-forays. No D-tier rule text changed. | D5's IQR clause is not gated at placement (not monotone) — a run can still fail it; watch attempt 6 |
