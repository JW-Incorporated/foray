...
| F-73 | tape-first windows (60–120 s) fail the D-tier duration rules by construction (D3 mean ≥ 90 s, D5 IQR ≥ 45 s, D2 recovery) and M4's runtime clause; attempt 5's partial refused | Critical | fix in flight | `fix/f73-f74-duration-rules-dai` (agent PR) | rule changes for generated Forays are founder-visible |
| F-74 | rule #65 refuses every segment from a `dai_suspected` show (146 of 220) although tier-2 segments now carry phrase anchors | Critical | fix in flight | same PR: #65 accepts anchored segments | founder-visible rule change |
