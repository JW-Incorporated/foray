...
| **3. Texture on a cadence** | A same-episode silent bridge that has let more than `TEXTURE_CADENCE_SEC = 155` seconds elapse since the last audible marker gets a jingle with `reason: "cadence"`. **155 is measured, not guessed**: `tools/foray/measure-cadence.mjs`, run against the real 61-minute `grilling-history-1` Foray, found 17 cut-gaps with a **median of 155.34 s** and a mean of 216.06 s. The median is used because it is least distorted by a handful of long single-episode stretches. That Foray was retired from `data/` on 2026-09-22 and is frozen verbatim in `tools/foray/fixtures/frozen/`; the script reads that copy, and `tools/foray/frozen-fixture.test.mjs` re-measures it and pins the constant, so the number stays reproducible. Measure a new long Foray (`--root`/`--foray`) to follow newer curation. |
...
| **D5** | `D5_TOLERANCE = 0.2` | **no two consecutive clips within ±20 % of the same length** (`max/min ≤ 1.2`, `d5UniformPairs`; mirrored in `backend/src/generation/d5Pair.ts`). **Reported, never gated (F-102)** — every uniform pair is a warning on every Foray and the count is in the report (`d5_uniform_pairs`), alongside the interquartile range (`d5_iqr_sec`). It was an error on Q-01-cut tape for one day: Q-01 says a clip runs as long as the tape stays relevant and Q-04 says two clips in a row must not be the same length, and a gate can only settle that by shortening a clip or refusing its tape — measured, four seeded beats refused and nine clips placed against a floor of ten. Sourcing therefore refuses nothing for it either (`placementAllows` no longer asks; `lengthGate: "d5-pair"` on the relevance row is a mark). Was: gated on `boundary`-carrying rows (`d5_gated`, removed); before that the uniform-triple clause plus `D5_IQR_FLOOR_SEC = 45`, served in sourcing by the 105/165/135/210 s ladder (F-73/F-80) |
...
| **Q-01 row fields** | `SEGMENT_BOUNDARIES = [turn, sentence, claim-only]` | a played row may carry `boundary` (one of the three) and `extended_by_sec` (a non-negative number), written by the tier-2 mint (`tapeExtent.ts` via `finalizeForay.ts`). Validated where present; **no rule turns on their presence** since F-102 removed D5's Q-01 gate, which was their only reader |
...
`d1_max_starts_in_window`, `d5_iqr_sec`, `d5_uniform_pairs`. (`d5_gated` was
removed by F-102 along with the gate it described.)
